...
title: Vue 3のsetInterval多重起動で単語が大量生成された話
date: 2026-05-10
section: guide
description: Vue 3 + TypeScriptのタイピングゲームで、再スタートのたびに単語の出現速度が上がっていく不具合が発生しました。原因のsetInterval多重起動と、タイマーを状態として管理する修正方法をまとめました。
tags: Vue 3, TypeScript, setInterval
---

# Vue 3のsetInterval多重起動で単語が大量生成された話

Vue 3 + TypeScriptでタイピングゲームを作っていて、ゲームを再スタートするたびに単語の出現速度が上がっていく不具合に当たりました。

原因は `setInterval` の多重起動です。単語生成の処理そのものを疑って読んでいたので、特定までに少し時間がかかりました。
...
この不具合が分かりにくかったのは、症状が「単語生成がおかしい」に見えることです。生成処理も単語リストも正しく、間違っていたのは呼び出し回数の管理だけでした。速度が再スタート回数に比例していることに気づいてから、疑う場所がタイマー側へ移りました。

対処自体は `clearInterval` を呼ぶだけですが、そのためにタイマーIDを状態として持つ必要があります。開始・停止・破棄時の解放を1組として設計しないと、`setInterval` を書いた時点では動いてしまうので、再スタートを試すまで気づけません。
