...
- ログインユーザー向けスコアAPIで401になった場合は、期限切れtokenまたは不正tokenとしてFE側のログイン状態をクリアし、共通アラートで再ログイン案内を表示する。プレイ結果はlocalStorage側に残す。
...
- 401 / token期限切れ時の扱いを整理する。ログインユーザー向けスコアAPIではログイン状態クリアと再ログイン案内まで実装済み。
```
